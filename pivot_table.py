...
group1.agg({'mpg':np.mean, 'wt':[np.mean, np.median, np.std], 'hp':'std'})
...
pd.crosstab([df1.cyl,df1.carb], [df1.gear, df1.am])
...
df1.pivot_table(index=['gear'], columns=['cyl'],  aggfunc ={'mpg':np.mean,'wt':[min,max,np.mean]})
